...
object Routes {
  ...
  private def runProcess(cmd: String, userIp: String, processNum: Int): IO[String] = IO {
    ...
    state.dequeueCommand match {
      case Some((nextCmd, nextIp)) =>
        ...
        runProcess(nextCmd, nextIp, newProcessNum).start.flatMap(_ => IO.pure(output))
      case None =>
        IO.pure(output)
    }
  }
